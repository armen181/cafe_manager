$(document).ready(function () {
    ...
    $('body').on("click", "#creatTableBtn", function (e) {
        $('#createTable').modal({
            backdrop: 'static',
            keyboard: false
        }, 'show');
    ...
    //===========  table create model ====================
    $('body').on("click", "#createTableButton", function (e) {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": "/rest/tableCreate",
            "method": "GET",
            "headers": {
                "name": $('#nameForCreateTable').val()
            }
        };
    ...
    //===========  table remove model ====================
    $('body').on("click", "#removeTableButton", function (e) {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": "/rest/tableRemoveById",
            "method": "GET",
            "headers": {
                "id": $('#tableIdForRemove').text()
            }
        };
        $.ajax(settings).done(function (response) {
            updateTable();
    ...
    //===========  open  Table Sign model to User ====================
    $('body').on("click", ".signToUser", function (e) {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": "/rest/userGetAll",
        ...
        $.ajax(settings).done(function (response) {
            ...
            $('#signUser').replaceWith(html + "</select>");
        });

        $('#signTableNameForUser').text(e.currentTarget.value);
        $('#signTableToUser').modal({
            backdrop: 'static',
            keyboard: false
        }, 'show');
    ...
    $('body').on("click", "#signTableToUserButton", function (e) {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": "/rest/tableSignUser",
            "method": "GET",
            "headers": {
                "cafeTableId": $('#signTableNameForUser').text(),
                "waiterId": $('#signUser').val()
            }
        };
